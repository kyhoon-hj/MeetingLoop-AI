...
import { getMinutesRevisions } from "@meetingloop/db";
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    const revisions = await getMinutesRevisions(session.userId, session.organizationId, meetingId);
    return NextResponse.json({ revisions }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    const response = databaseErrorResponse(error);
    if (response) return response;
    logUnexpectedServerError("minutes.revisions.get", error);
